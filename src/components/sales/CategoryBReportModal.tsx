...
const MOCK_INVOICES = [
  ...
  {
    id: "SKU: OV-001",
    name: "Overflow Invoice #1",
    taxStatus: "Overflow",
    total: 12500,
  },
];

export default function CategoryBReportModal({
  isOpen,
  onClose,
  onPrintPDF,
  data,
      ...
      <div className="relative bg-white w-full max-w-[560px] rounded-[28px] shadow-2xl overflow-hidden mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="bg-gradient-to-br from-[#15803d] to-[#166534] p-8 pb-7 text-white flex-shrink-0">
          ...
          <h2 className="text-[22px] font-black tracking-tight mb-1">
            View Detailed Report (Category B)
          </h2>
          <p className="text-[13px] font-bold text-emerald-200">
            Non-taxable items &amp; overflow above Rs. 200,000
          </p>
        ...
        <div className="flex-1 overflow-y-auto p-7">
              ...
              <tbody>
                {filtered.map((inv: any, i: number) => (
                  <tr key={i} className="border-b border-gray-50 last:border-0">
                    ...
                    <td className="py-4 px-4">
                      <span
                        className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${inv.type !== "Overflow" ? "bg-emerald-50 text-emerald-600" : "bg-blue-50 text-blue-600"}`}
                      >
                        {inv.type}
                      </span>
                    </td>
                    <td className="py-4 px-4 text-right text-[13px] font-black text-emerald-600 font-mono">
                      Rs. {inv.rawAmount?.toLocaleString() || inv.amount}
                    </td>
                    <td className="py-4 px-4 text-right">
                      <span
                        className={`px-2 py-1 text-[11px] font-black rounded-lg ${inv.status === "Completed" || inv.status === "Paid" ? "bg-emerald-100 text-emerald-700" : inv.status === "Refunded" ? "bg-amber-100 text-amber-700" : "bg-gray-100 text-gray-700"}`}
                      >
          ...
          <div className="space-y-3 border-t border-gray-100 pt-5">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="px-4 py-1.5 bg-emerald-50 text-emerald-600 text-[11px] font-black rounded-full uppercase tracking-wider">
                Non-Taxable & Overflow
              </span>
            </div>
        ...
        <div className="flex gap-3 p-6 pt-4 border-t border-gray-100 flex-shrink-0">
          ...
          <button
            ...
            className="flex-1 py-3.5 bg-[#15803d] hover:bg-emerald-800 rounded-2xl text-[14px] font-black text-white shadow-lg shadow-emerald-100 transition-all active:scale-95"
          >
            View All
          </button>
        </div>
