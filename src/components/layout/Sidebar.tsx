...
const menuItems = [
  {
    ...
    label: "Dashboard",
    href: "/dashboard",
    roles: ["admin", "owner", "manager", "staff", "cashier"],
  },
  {
    icon: Printer,
    label: "Point of Sale",
    href: "/pos",
    roles: ["admin", "owner", "manager", "cashier", "staff"],
  },
  {
    icon: Boxes,
    label: "Inventory",
...
export default function Sidebar({
  ...
  const visibleMenuItems = menuItems.filter(
    (item) => user?.role && item.roles.includes(user.role as any),
  );
  ...
  return (
    <>
      ...
      >
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center justify-center shrink-0">
                <img
                  ...
                  alt="Futura Hardware Logo"
                  className="h-12 sm:h-16 object-contain"
                />
              </div>
              <div className="min-w-0">
                <h1 className="text-[15px] font-bold tracking-wide leading-tight uppercase text-white">
                  FUTURA HARDWARE
                </h1>
                <p className="text-[11px] text-white/70 font-normal mt-0.5">
                  Management System
                </p>
              </div>
            </div>
        ...
        {/* Navigation */}
        <nav
          className="flex-1 px-3 space-y-1 pb-6 overflow-y-auto overflow-x-hidden"
            ...
            <div className="flex items-center gap-3 cursor-pointer">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-[14px] font-bold truncate text-white leading-tight">
                  {user?.name ||
                    (isAuthenticated ? "Connecting..." : "Guest User")}
                </span>
