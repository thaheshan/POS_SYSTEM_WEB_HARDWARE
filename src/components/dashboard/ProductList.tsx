      ...
      <div className="bg-white rounded-[24px] p-8 shadow-sm border border-gray-100 min-w-[380px] flex flex-col">
        <div className="flex justify-between items-center mb-8">
          ...
          <button 
            ...
            className="text-[13px] font-bold text-blue-600 hover:text-blue-700 transition-colors uppercase tracking-wide cursor-pointer"
          >
            View All →
          </button>
        </div>
            ...
            products.map((product, index) => {
              ...
              return (
                <div 
                  ...
                  onClick={() => setIsModalOpen(true)}
                  className="flex items-center gap-4 group cursor-pointer p-2 -mx-2 rounded-xl hover:bg-blue-50/50 transition-colors"
                >
                  <div className={cn("w-12 h-12 rounded-[14px] flex items-center justify-center shrink-0 transition-transform group-hover:scale-105", bg)}>
                    <Icon className={cn("w-5 h-5", color)} strokeWidth={2.5} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="text-[14px] font-bold text-gray-900 tracking-tight truncate group-hover:text-blue-600 transition-colors">
                      {product.name || (product as any).product_name}
                    </h4>
