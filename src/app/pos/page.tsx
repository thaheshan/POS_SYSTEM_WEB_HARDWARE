...
function QtyPopup({
  ...
  const [qty, setQtyLocal] = useState<number | string>(
    currentQty > 0 ? currentQty : isLoose ? "" : 1,
  );
        ...
        >
          <div className="flex items-center gap-4 p-5 bg-gray-50 border-b border-gray-100">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[10px] font-black text-[#059669] uppercase tracking-widest mb-1">
                {product.category}
              </p>
              <h3 className="text-[15px] font-black text-gray-900 leading-snug mb-1 line-clamp-2">
                {product.name}
              </h3>
              <p className="text-[12px] font-bold text-gray-400">
                Rs. {product.price.toLocaleString()} /{" "}
                {isLoose ? product.measurementUnit || "unit" : "unit"}
              </p>
              <p className="text-[11px] font-bold text-amber-600 mt-1">
                Available: {product.stock}{" "}
                {isLoose ? product.measurementUnit : ""}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 space-y-5">
            <div>
              <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest mb-2">
                {isLoose
                  ? `Enter Measurement (${product.measurementUnit || "kg/m"})`
                  : "Enter Quantity"}
              </p>
              <div className="flex items-center gap-4">
                ...
                {!isLoose && (
                  <button
                    onClick={() =>
                      setQtyLocal((q) => (typeof q === "number" ? q : 1) + 1)
                    }
            ...
            <div className="bg-emerald-50 border border-emerald-100 rounded-2xl px-5 py-4 flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold text-gray-400">
                  {parsedQty} × Rs. {product.price.toLocaleString()}
                </p>
                <p className="text-[11px] font-black text-emerald-700 uppercase tracking-widest mt-0.5">
                  Line Total
                </p>
              </div>
              <span className="text-[22px] font-black text-[#059669]">
...
export default function POSPage() {
  ...
  const categories = Array.from(
    new Set(["All", ...(categoriesData?.map((c) => c.categoryName) || [])]),
  );
  ...
  const fetchProducts = async () => {
    try {
      ...
      const stockProductIds = new Set(
        stockItems.map((s: any) => String(s.product_id || s.productId)),
      );

      // Map products that have stock records
      const mappedStockProducts: Product[] = stockItems.map(
        (item: any, index: number) => {
          ...
          return {
            ...
            category:
              item.product?.category?.name || item.category_name || "All",
            img:
              item.image_url ||
  ...
  const ensureMinQty = (id: string) => {
    setCart((prev) =>
      prev.map((item) =>
        item.id === id ? { ...item, qty: Math.max(1, item.qty) } : item,
      ),
    );
  };
  ...
  useEffect(() => {
    const handleGlobalKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "F2") {
        e.preventDefault();
        setActiveTab((prev) => (prev === "items" ? "checkout" : "items"));
      }
      if (e.key === "F9" && activeTab === "checkout") {
        e.preventDefault();
                        ...
                        >
                          <div className="h-[280px] w-full bg-gray-100 relative overflow-hidden">
                            ...
                            <div className="absolute top-3 right-3">
                              <span
                                className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md shadow-sm border ${
              ...
              <div className="w-[350px] md:w-[400px] lg:w-[450px] xl:w-[500px] shrink-0 bg-white flex flex-col shadow-2xl z-20 border-l border-gray-200 h-full overflow-hidden">
                ...
                <div className="flex bg-gray-50 p-1.5 m-3 rounded-2xl border border-gray-100 gap-1 shrink-0">
                  <button
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-xl text-[12px] font-black tracking-tight transition-all duration-200 ${
                      activeTab === "items"
                        ? "bg-white text-[#059669] shadow-md border border-emerald-100"
                ...
                <div className="flex-1 overflow-y-auto scroll-smooth">
                  ...
                  {activeTab === "items" && (
                    <div className="p-5 space-y-5">
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={() => setCart([])}
                          className="text-[11px] font-bold text-red-400 hover:text-red-500 hover:bg-red-50 px-3 py-1.5 rounded-lg transition-colors"
                        >
                          Clear All
                        </button>
                      </div>
                        ...
                        <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-amber-700 mt-0.5">
                              Some items in basket require immediate reordering.
                            </p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="space-y-3">
                        {cart.map((item) => (
                          <div
                            key={item.id}
                            className="flex gap-3 p-4 rounded-2xl bg-gray-50 border border-gray-100 relative group hover:bg-white hover:shadow-md transition-all"
                          >
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-1.5">
                                  <button
                                    ...
                                    className="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 active:scale-90 transition-all border border-gray-200"
                                  >
                                  ...
                                  <button
                                    onClick={() => updateQty(item.id, 1)}
                                    className="w-10 h-10 rounded-xl bg-[#059669] flex items-center justify-center text-white hover:bg-emerald-700 active:scale-90 transition-all shadow-sm"
                                  >
                            ...
                            <button
                              ...
                              className="absolute top-2 right-2 p-1 text-red-300 opacity-0 group-hover:opacity-100 transition-opacity hover:text-red-500"
                            >
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                  ...
                  {activeTab === "checkout" && (
                    <div className="p-5 space-y-6">
                      {/* Customer */}
                      <div className="space-y-3">
                        <h3 className="text-[11px] font-black text-gray-400 uppercase tracking-[0.15em] flex items-center gap-2">
                            ...
                            <button
                              ...
                              className={`flex flex-col items-center gap-2 py-4 rounded-2xl border-2 transition-all ${
                                paymentMethod === method.id
                                  ? "bg-[#059669] border-[#059669] text-white shadow-lg scale-105"
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="relative">
                          <span className="absolute left-4 top-1/2 -translate-y-1/2 text-emerald-600 font-black text-[16px]">
                            Rs.
                          </span>
                          <input
                            type="number"
                        ...
                        <div className="space-y-3 p-4 bg-gray-50 rounded-2xl border border-gray-100 mb-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className={`flex-1 py-2 rounded-lg text-[11px] font-black uppercase ${discountType === "percentage" ? "bg-[#059669] text-white" : "bg-white text-gray-400 border border-gray-200"}`}
                            >
                              % Percent
                            </button>
                            <button
                              onClick={() => setDiscountType("fixed")}
                              className={`flex-1 py-2 rounded-lg text-[11px] font-black uppercase ${discountType === "fixed" ? "bg-[#059669] text-white" : "bg-white text-gray-400 border border-gray-200"}`}
                            >
                              Amount
                            </button>
                          </div>
                          <div className="flex gap-2">
                            ...
                            {discountValue > 0 && (
                              <button
                                ...
                                className="bg-red-50 text-red-600 border border-red-100 hover:bg-red-100 px-4 rounded-lg text-[11px] font-black uppercase shrink-0 transition-colors"
                              >
                                Clear
                              </button>
                            )}
                ...
                <div className="bg-white border-t border-gray-100 shadow-[0_-8px_30px_rgba(0,0,0,0.06)] shrink-0">
                  ...
                  <div className="px-4 py-3 bg-emerald-50/50 flex items-center justify-between border-y border-emerald-100">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#059669] opacity-60">
                        Total Payable
                      </p>
                    </div>
                    <span className="text-[26px] font-black tracking-tighter text-[#059669]">
                      Rs. {total.toLocaleString()}
                    </span>
                  </div>
                  <div className="p-4 space-y-2.5">
                    ...
                    >
                      <CheckCircle2
                        className={`w-5 h-5 ${activeTab === "items" ? "opacity-20" : ""}`}
                      />
                      COMPLETE SALE
                    </button>
                    {activeTab === "items" && (
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        ...
                        {heldOrders.length > 0 && `(${heldOrders.length})`}
                      </button>
                      <button
                        onClick={handlePrint}
                        className="flex-1 bg-gray-50 border border-gray-100 hover:bg-gray-100 text-gray-500 py-2.5 rounded-xl font-black text-[11px] uppercase tracking-widest transition-colors flex items-center justify-center gap-1.5"
