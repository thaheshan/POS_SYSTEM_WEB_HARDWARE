...
export default function StaffManagementPage() {
  ...
  const handleApprove = async (
    ...
    if (!confirm(`Are you sure you want to ${verb} this registration request?`))
      return;

    // Owner-only guard (support either 'owner' or 'OWNER' depending on backend)
    const role = user?.role;
    const roleKey = String(user?.role ?? "").toUpperCase();
    if (roleKey !== "OWNER" && roleKey !== "SUPER_ADMIN") {
      alert("Only shop owners can perform approval actions.");
      return;
    }

    try {
      setApprovingId(staffId);
      const res = await api.post("/staff/approve", {
        staff_id: staffId,
        action,
      });
      // Prefer server message when available
      const msg = res?.data?.message || `Request ${verb}d`;
      await fetchAll(); // refresh lists
      alert(msg);
    } catch (err: any) {
      console.error(`[Staff] Failed to ${action}`, err);
      const status = err?.response?.status;
      const dataMsg = err?.response?.data?.message;
      if (status === 403) {
        alert(dataMsg || "You are not authorized to perform this action.");
      } else if (status === 400) {
        alert(
          Array.isArray(dataMsg)
            ? dataMsg.join(", ")
            : dataMsg || `Invalid request to ${verb}.`
        );
      } else {
        alert(dataMsg || `Failed to ${verb} request. Please try again later.`);
      }
    } finally {
      setApprovingId(null);
    }
  ...
  const adminCount = staff.filter(
    (s) => s.role === "OWNER" || s.role === "SUPER_ADMIN"
  ).length;
      ...
      <MainLayout>
        <div className="max-w-[1600px] mx-auto pb-20 animate-pulse">
          ...
          <div className="grid grid-cols-5 gap-4 mb-8">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="h-32 bg-gray-100 rounded-[20px]" />
            ))}
          </div>
    ...
    <MainLayout>
      <div className="max-w-[1600px] mx-auto pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[14px] font-medium text-gray-500 tracking-wide mt-1">
              Manage roles, permissions, attendance, and staff accounts
            </p>
          </div>
        ...
        <div className="flex items-center gap-2 mb-6 border-b border-gray-200 pb-px overflow-x-auto">
          {["All Staff", "Admin", "Manager", "Cashier", "Pending"].map(
            (tab) => {
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-6">
            ...
            <div className="flex items-center gap-4 w-full lg:w-auto justify-end">
              ...
              <div className="flex items-center border border-gray-200 rounded-[10px] p-1 bg-gray-50">
                <button className="p-1.5 bg-white text-blue-600 shadow-sm rounded-md">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
            {isLoading ? (
              ...
              filteredStaff.map((s) => (
                <div
                  key={s.id}
                  className="bg-white rounded-[20px] shadow-sm flex flex-col overflow-hidden transition-all hover:shadow-md border border-gray-200 relative"
                >
                  <div
                    className={`h-1.5 w-full ${
                      accentColor[s.role] ?? "bg-gray-300"
                    }`}
                  />
                  <div className="p-6 pb-4">
                    <div className="flex justify-between items-start mb-6">
                      <div className="flex gap-4 items-center">
                        <div
                          className={`w-12 h-12 rounded-full flex items-center justify-center text-[16px] font-black shrink-0 ${
                            avatarBg[s.role] ?? "bg-gray-100 text-gray-600"
                          }`}
                        >
                        ...
                        <div>
                          ...
                          <p className="text-[11px] font-bold text-gray-400 font-mono mt-0.5">
                    ...
                    <div className="flex items-center gap-2 mb-6">
                      <span
                        className={`px-2.5 py-1 rounded text-[10px] font-black tracking-widest uppercase flex items-center gap-1.5 ${
                          roleColor[s.role] ?? "bg-gray-50 text-gray-600"
                        }`}
                      >
                        {s.role === "CASHIER" && (
        ...
        {activeTab === "Pending" && (
          <>
            <div className="flex justify-between items-end mb-4 border-b border-gray-200 pb-4">
              <h2 className="text-[18px] md:text-[22px] font-black text-gray-900 tracking-tight">
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-gray-50/70 border-b border-gray-100">
                    <th className="py-4 px-6 text-[12px] font-black text-gray-500 tracking-wider w-[60px]">
                      Staff
                    </th>
                    <th className="py-4 px-4 text-[12px] font-black text-gray-500 tracking-wider">
                      Details
                    </th>
                    <th className="py-4 px-4 text-[12px] font-black text-gray-500 tracking-wider">
                      Role
                    ...
                    <th className="py-4 px-6 text-[12px] font-black text-gray-500 tracking-wider text-right">
                      Actions
                    </th>
                  </tr>
                </thead>
                      ...
                      <tr key={req.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="py-4 px-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div>
                              <span className="text-[11px] font-black px-2 py-1 rounded-full bg-yellow-50 text-amber-700 border border-amber-100">
                                {req.status ?? "Pending"}
                              </span>
                            </div>
                          </div>
                        ...
                        <td className="py-4 px-4">
                          <p className="text-[12px] font-bold text-gray-700 font-mono mb-0.5">
                            {req.shopId?.slice(0, 12) ?? "N/A"}
                          </p>
...
function AddStaffModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!shopId) {
        setError(
          "Shop ID (tenantId) is missing from your session. Please log out and log back in."
        );
        setLoading(false);
        return;
      }
