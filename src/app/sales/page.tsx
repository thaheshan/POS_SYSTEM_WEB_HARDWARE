...
export default function SalesPage() {
  ...
  const handleExportCSV = () => {
    ...
    if (isAdmin) {
      csvRows.push(['Category A', 'Taxable Sales', data.catA.core, data.catA.txns]);
      csvRows.push(['Category A', 'VAT (18%)', data.catA.vat, '']);
      csvRows.push(['Category B', 'Non-Tax & Overflow', data.catB.core, data.catB.txns]);
      csvRows.push(['Category B', 'Overflow (>2L)', data.catB.overflow, '']);
    }

    csvRows.push(['Category C', 'Labour & Misc', data.catC.core, data.catC.entries]);
    csvRows.push(['Category C', 'Labour Charges', data.catC.labour, '']);

    if (isAdmin) {
      csvRows.push(['', 'Total Combined', data.catA.core + data.catB.core + data.catC.core, '']);
    } else {
      csvRows.push(['', 'Total (Cat C)', data.catC.core, '']);
    }
    ...
    <>
      <MainLayout>
        <div className="max-w-[1600px] mx-auto pb-20 print:hidden">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
            <div>
              <h1 className="text-[28px] font-black text-gray-900 tracking-tight mb-1">
                {isAdmin ? 'Sales Dashboard - Tax Category View' : 'Sales Dashboard - Operations View'}
              </h1>
              <p className="text-[14px] font-bold text-gray-400 opacity-80">
                {isAdmin 
                  ...
                  : 'Operational Labour & Miscellaneous Sales Tracking'}
              </p>
            </div>
            ...
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Portal>
                <DropdownMenu.Content
                  className="bg-white min-w-[180px] p-2 rounded-xl shadow-xl border border-gray-100 z-50 animate-in slide-in-from-top-1 duration-200 print:hidden"
                  sideOffset={8}
                  align="end"
                >
                  <DropdownMenu.Item
          ...
          {/* TAXABLE STATUS BANNER - Admin Only */}
          {isAdmin && (
          ...
          <div className={cn(
            "grid gap-6 items-stretch relative",
            isAdmin ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1 max-w-2xl mx-auto"
          )}>

            {/* CATEGORY A: TAXABLE SALES (BLUE) - Admin Only */}
            {isAdmin && (
              <div className="bg-white rounded-[24px] border border-gray-100 shadow-xl overflow-hidden flex flex-col h-full min-h-[850px]">
                <div className="p-6 pb-8 bg-gradient-to-br from-[#1e40af] to-[#1e3a8a] text-white">
                  <span className="text-[11px] font-black uppercase tracking-[0.15em] bg-white/20 px-3 py-1 rounded-md">
                    Category A
                  </span>
                  <h3 className="text-[20px] font-black mt-4 mb-1">Taxable Sales</h3>
                  <p className="text-[12px] font-medium opacity-80 mb-6 leading-tight">
                ...
                <div className="p-6 space-y-6 bg-white flex-1">
                  <div className="grid grid-cols-2 gap-4 pb-6 border-b border-gray-50">
                    <div>
                      <span className="text-[11px] font-bold text-gray-400 uppercase leading-tight mb-2 block">
                        Average Bill
                      </span>
            ...
            {/* CATEGORY B: NON-TAX & OVERFLOW (GREEN) - Admin Only */}
            {isAdmin && (
              <div className="bg-white rounded-[24px] border border-gray-100 shadow-xl overflow-hidden flex flex-col h-full min-h-[850px]">
                ...
                <div className="px-6 py-4 border-b border-gray-50 space-y-2 bg-gray-50/30">
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-400">
                    ...
                    <span className="text-gray-900 font-mono font-bold">
                      Rs. {data.catB.overflow.toLocaleString()}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-400">
                    <span>Non-taxable Products</span>
                    <span className="text-gray-900 font-mono font-bold">
                      Rs. {data.catB.baseNonTax.toLocaleString()}
                    </span>
                  </div>
                </div>
                ...
                <div className="p-6 space-y-6 flex-1 bg-white">
                  <div className="grid grid-cols-2 gap-4 pb-6 border-b border-gray-50">
                    <div>
                      ...
                      <span className="text-[16px] font-black text-gray-900 font-mono tracking-tighter">
                        Rs. {data.catB.avg.toLocaleString()}
                      </span>
                    </div>
                    <div>
                      <span className="text-[11px] font-bold text-gray-400 uppercase leading-tight mb-2 block">
                  ...
                  <div>
                    ...
                    <div className="flex items-center justify-between mb-4 mt-8">
                      <h5 className="text-[11px] font-black uppercase tracking-widest text-gray-900 font-mono">
                        Recent Transactions
                      </h5>
                    </div>
                    <div className="space-y-4">
            ...
            <div className="bg-white rounded-[24px] border border-gray-100 shadow-xl overflow-hidden flex flex-col h-full min-h-[850px]">
              ...
              <div className="px-6 py-4 border-b border-gray-50 space-y-2 bg-gray-50/30">
                <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  ...
                  <span className="text-gray-900 font-mono font-bold">
                    Rs. {data.catC.labour.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Installation Fees</span>
                  <span className="text-gray-900 font-mono font-bold">
                    Rs. {data.catC.install.toLocaleString()}
                  </span>
                </div>
                <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-gray-400">
                  <span>Other Expenses</span>
                  <span className="text-gray-900 font-mono font-bold">
                    Rs. {data.catC.misc.toLocaleString()}
                  </span>
                </div>
              </div>
        ...
        <CategoryAReportModal
          ...
          onPrintPDF={(timeFilter) => {
            ...
            setTimeout(() => { window.print(); setPrintCategory(null); }, 100);
          }}
          data={data}
        />
